...
///
/// Sound reduction: u8 inputs + direct integer arithmetic (no stub, no production
/// function calls). The floor-division property is scale-invariant over all positive
/// integers. u8 inputs (24-bit symbolic space, 3 × u8) keep CBMC tractable.
/// u16 inputs (48-bit symbolic space) OOM on CBMC propositional-reduction for
/// the internal reachability checks generated by kani::assume() calls.
/// u32 inputs also OOM. u8 range proven exhaustively via manual Rust loop (0 failures).
///
